...
void *baseRateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    cartesian_trajectory_planner_2_step();
...
{
  UNUSED(sig);
  rtmSetErrorStatus(cartesian_trajectory_planner_M, "stopping the model");
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  cartesian_trajectory_planner_2_terminate();
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(cartesian_trajectory_planner_M, 0);

  // Initialize model
  cartesian_trajectory_planner_2_initialize();
